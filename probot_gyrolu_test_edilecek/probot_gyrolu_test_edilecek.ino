...
float autoTurnTargetYaw = 0.0f;
bool autoTurnArmed = false;
float autoTurnPrevYaw = 0.0f;
...
const unsigned long AUTO_TURN_TIMEOUT_MS = 5000;
const unsigned long AUTO_LIFT_MS = 2000;

// #region agent log
void debugLog(const char* hypothesisId, const char* location, const char* message,
              float v1 = 0, float v2 = 0, float v3 = 0, float v4 = 0) {
  char buf[192];
  snprintf(buf, sizeof(buf),
    "{\"sessionId\":\"5bb6af\",\"hypothesisId\":\"%s\",\"location\":\"%s\","
    "\"message\":\"%s\",\"data\":{\"v1\":%.2f,\"v2\":%.2f,\"v3\":%.2f,\"v4\":%.2f},"
    "\"timestamp\":%lu}",
    hypothesisId, location, message, v1, v2, v3, v4, millis());
  probot::telemetry::println(buf);
}
// #endregion
...
void turnInPlace(float turnPower) {
  writeSpark(M_FL, turnPower);  // mekanum + capraz kablo: hepsi ayni = yerinde donus
...
void autonomousInit() {
  ...
  // #region agent log
  updateGyro();
  debugLog("A", "autonomousInit", "otonom basladi", currentYaw, gyroReady ? 1.0f : 0.0f,
           (float)gyroUpdateCount, (float)(millis() - lastGyroUpdateMs));
  // #endregion
}
...
void autonomousLoop() {
  ...
  switch (autoStep) {
    ...
    case 3: {
      if (!autoTurnArmed) {
        autoTurnTargetYaw = normalizeAngle(currentYaw + 90.0f);
        autoTurnPrevYaw = currentYaw;
        autoTurnArmed = true;
        autoTimer = millis();
        // #region agent log
        debugLog("C", "case3:arm", "donus hedefi kuruldu", currentYaw, autoTurnTargetYaw,
                 90.0f, gyroReady ? 1.0f : 0.0f);
        // #endregion
      }

      float yawError = normalizeAngle(autoTurnTargetYaw - currentYaw);
      bool timedOut = (millis() - autoTimer) >= AUTO_TURN_TIMEOUT_MS;
      bool reached = fabs(yawError) <= AUTO_TURN_TOLERANCE;

      if (reached || timedOut) {
        stopMotors();
        // #region agent log
        debugLog(reached ? "D" : "E", "case3:done", reached ? "hedefe ulasildi" : "timeout",
                 currentYaw, autoTurnTargetYaw, yawError, (float)(millis() - autoTimer));
        // #endregion
        autoTurnArmed = false;
        autoTimer = millis();
        autoStep = 4;
      } else {
        float turnPower = yawError * 0.025f;
        if (fabs(yawError) < 25.0f) {
          turnPower *= 0.45f;
        }
        turnPower = constrain(turnPower, -AUTO_TURN_POWER, AUTO_TURN_POWER);
        if (fabs(turnPower) < 0.15f) {
          turnPower = (yawError > 0.0f) ? 0.15f : -0.15f;
        }
        turnInPlace(turnPower);
        // #region agent log
        static unsigned long lastCase3LogMs = 0;
        if (millis() - lastCase3LogMs >= 150) {
          float yawDelta = normalizeAngle(currentYaw - autoTurnPrevYaw);
          lastCase3LogMs = millis();
          debugLog("F", "case3:loop", "donus devam", currentYaw, yawError, turnPower, yawDelta);
          autoTurnPrevYaw = currentYaw;
        }
        // #endregion
      }
